									...
									<table class="rgMasterTable rgClipCells d_check" id="ctl00_ContentPlaceHolder1_RadGridEmployee_ctl00" style="width:100%;table-layout:fixed;overflow:hidden;empty-cells:show;">
										...
										<c:forEach items="${requestScope.e_list}" var="evo">
											<tr class="rgRow" id="ctl00_ContentPlaceHolder1_RadGridEmployee_ctl00__0" style="white-space:nowrap;">
												...
												<td class="col_ct">${fn:substring(evo.hire_date,0,4)-fn:substring(evo.con_year,0,4)}</td>
											</tr>
										</c:forEach>
										<!-- 사원 리스트 -->
										</c:if>
	...
	$(function(){
		...
		function showData(empno){
			...
			$.ajax({
				...
				$("#dept_name").val("");
				if(res.evo.dvo){
				$("#dept_name").val(res.evo.dvo.dept_name); //부서
				$("#dept_code").val(res.evo.dept_code);	
				}
				
				$("#pos_name").val("");			
				if(res.evo.pvo){
				$("#pos_name").val(res.evo.pvo.pos_name); //직급
				$("#pos_code").val(res.evo.pos_code);	
				}
				
				$("#duty_name").val("");						
				if(res.evo.dutyvo){
				$("#duty_name").val(res.evo.dutyvo.duty_name); //직책
